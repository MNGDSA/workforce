#!/usr/bin/env tsx
// Task #200 — backfill helper for the contract-template-logo private-file
// bug. Before this task, `POST /api/contract-templates/:id/logo` uploaded
// the file to DigitalOcean Spaces with the default "private" ACL, so the
// browser received 403 on every render of the contract preview / PDF.
// The forward fix in `server/routes.ts` now passes `{ isPublic: true }`,
// but every logo uploaded before the fix is still private at rest.
//
// This script flips the ACL on existing `contract_templates.logo_url`
// objects from "private" to "public-read" using `PutObjectAclCommand`.
// It does NOT re-upload the bytes — only the ACL is modified, so the
// existing URLs remain valid.
//
// Usage:
//   tsx scripts/backfill-public-logos.ts            # dry-run (default)
//   tsx scripts/backfill-public-logos.ts --apply    # apply changes
//
...
//
// In dev (no `NODE_ENV=production`) all logo URLs look like
// `/uploads/<filename>` and live on local disk — there is nothing to flip
// and the script exits with a no-op message.
...
async function main() {
  ...
  const rows = await db
    .select({ id: contractTemplates.id, name: contractTemplates.name, logoUrl: contractTemplates.logoUrl })
    .from(contractTemplates)
    .where(isNotNull(contractTemplates.logoUrl));

  if (rows.length === 0) {
    console.log("No contract templates with a logoUrl found. Nothing to do.");
    return;
  }

  const remoteRows = rows.filter((r) => r.logoUrl && !r.logoUrl.startsWith("/uploads/"));
  const localRows = rows.filter((r) => r.logoUrl && r.logoUrl.startsWith("/uploads/"));

  console.log(`Found ${rows.length} contract template(s) with a logoUrl:`);
  ...
  for (const row of remoteRows) {
    let key: string;
    try {
      key = extractStorageKeyFromUrl(row.logoUrl!);
    } catch (err: any) {
