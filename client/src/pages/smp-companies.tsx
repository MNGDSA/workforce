...
function CreateCompanyDialog({
  ...
  const { t } = useTranslation(["smpCompanies"]);
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: t("smpCompanies:create.successTitle") });
      reset();
      onOpenChange(false);
    },
    onError: () => toast({ title: t("smpCompanies:create.errorTitle"), description: t("smpCompanies:create.errorDesc"), variant: "destructive" }),
  });

  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.name.trim()) return toast({ title: t("smpCompanies:create.nameRequired"), variant: "destructive" });
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) reset(); onOpenChange(v); }}>
      <DialogContent className="max-w-xl bg-card border-border">
        <DialogHeader>
          <DialogTitle className="text-white font-display text-xl">{t("smpCompanies:create.title")}</DialogTitle>
          <DialogDescription className="text-muted-foreground text-sm">
            {t("smpCompanies:create.desc")}
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            <Label className="text-white text-sm">{t("smpCompanies:create.name")} <span className="text-red-400">*</span></Label>
            <Input
              value={form.name}
              onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
              placeholder={t("smpCompanies:create.namePh")}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <Label className="text-white text-sm">{t("smpCompanies:create.cr")}</Label>
              <Input
                value={form.crNumber}
                onChange={e => setForm(f => ({ ...f, crNumber: e.target.value }))}
                placeholder={t("smpCompanies:create.crPh")}
                dir="ltr"
            ...
            <div className="space-y-1.5">
              <Label className="text-white text-sm">{t("smpCompanies:create.region")}</Label>
          ...
          <div className="border-t border-border pt-4 space-y-3">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-widest">{t("smpCompanies:create.contactInfo")}</p>
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label className="text-white text-sm">{t("smpCompanies:create.contactPerson")}</Label>
                <Input value={form.contactPerson} onChange={e => setForm(f => ({ ...f, contactPerson: e.target.value }))} placeholder={t("smpCompanies:create.contactPersonPh")} className="bg-muted/30 border-border" data-testid="input-smp-contact-person" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-white text-sm">{t("smpCompanies:create.contactPhone")}</Label>
                <Input value={form.contactPhone} onChange={e => setForm(f => ({ ...f, contactPhone: e.target.value }))} placeholder={t("smpCompanies:create.contactPhonePh")} dir="ltr" className="bg-muted/30 border-border" data-testid="input-smp-contact-phone" />
              </div>
            </div>
            <div className="space-y-1.5">
              <Label className="text-white text-sm">{t("smpCompanies:create.contactEmail")}</Label>
              <Input type="email" value={form.contactEmail} onChange={e => setForm(f => ({ ...f, contactEmail: e.target.value }))} placeholder={t("smpCompanies:create.contactEmailPh")} dir="ltr" className="bg-muted/30 border-border" data-testid="input-smp-contact-email" />
            </div>
          </div>

          <div className="border-t border-border pt-4 space-y-3">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-widest">{t("smpCompanies:create.bankInfo")}</p>
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label className="text-white text-sm">{t("smpCompanies:create.bankName")}</Label>
                <Input value={form.bankName} onChange={e => setForm(f => ({ ...f, bankName: e.target.value }))} placeholder={t("smpCompanies:create.bankNamePh")} className="bg-muted/30 border-border" data-testid="input-smp-bank-name" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-white text-sm">{t("smpCompanies:create.bankIban")}</Label>
                <Input value={form.bankIban} onChange={e => setForm(f => ({ ...f, bankIban: e.target.value.toUpperCase() }))} placeholder={t("smpCompanies:create.bankIbanPh")} dir="ltr" className="bg-muted/30 border-border font-mono text-xs" data-testid="input-smp-bank-iban" />
              </div>
            </div>
          </div>

          <div className="space-y-1.5">
            <Label className="text-white text-sm">{t("smpCompanies:create.notes")}</Label>
            <Textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} placeholder={t("smpCompanies:create.notesPh")} className="bg-muted/30 border-border resize-none" rows={2} data-testid="textarea-smp-notes" />
          </div>

          <DialogFooter>
            <Button type="button" variant="ghost" onClick={() => { reset(); onOpenChange(false); }} className="text-muted-foreground">{t("smpCompanies:create.cancel")}</Button>
...
function DocumentVault({ company, events }: { company: SMPCompany; events: { id: string; name: string }[] }) {
  ...
  const createDocMutation = useMutation({
    ...
    onError: () => toast({ title: t("smpCompanies:docs.saveFail"), variant: "destructive" }),
  });
  ...
  const deleteDocMutation = useMutation({
    ...
    onError: () => toast({ title: t("smpCompanies:docs.deleteFail"), variant: "destructive" }),
  });
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      toast({ title: t("smpCompanies:docs.uploadedToast", { count: files.length, replace: { count: formatNumber(files.length, i18n.language) } }) });
      setDescription("");
      setEventId("");
    } catch (err: any) {
      toast({ title: t("smpCompanies:docs.uploadFail"), description: err?.message, variant: "destructive" });
    } finally {
      setUploading(false);
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-2 border border-border rounded-sm p-3 bg-muted/5">
        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-widest">{t("smpCompanies:docs.uploadHeader")}</p>
        <div className="grid grid-cols-2 gap-2">
          <div className="col-span-2">
            <Input
              placeholder={t("smpCompanies:docs.descriptionPh")}
              value={description}
          ...
          <div className="col-span-2">
            <Select value={eventId} onValueChange={setEventId}>
              <SelectTrigger className="bg-muted/30 border-border text-white text-sm" data-testid="select-doc-event">
                <SelectValue placeholder={t("smpCompanies:docs.eventPh")} />
              </SelectTrigger>
              <SelectContent className="bg-card border-border">
                <SelectItem value="_none" className="text-muted-foreground">{t("smpCompanies:docs.noEvent")}</SelectItem>
        ...
        >
          ...
          {uploading ? t("smpCompanies:docs.uploading") : t("smpCompanies:docs.chooseUpload")}
        </Button>
        <input
          ref={fileInputRef}
          type="file"
      ...
      <div className="space-y-1.5">
        {isLoading ? (
          ...
          <div className="text-center py-6 border border-dashed border-border rounded-sm">
            ...
            <p className="text-xs text-muted-foreground">{t("smpCompanies:docs.noDocs")}</p>
          </div>
        ) : (
          docs.map((doc) => {
            ...
            <div className="p-4">
              {isImage(previewDoc.fileUrl) ? (
                ...
                <div className="text-center py-12 text-muted-foreground">
                  ...
                  <p className="text-sm">{t("smpCompanies:docs.previewUnavailable")}</p>
                  <a href={previewDoc.fileUrl} download={previewDoc.fileName} target="_blank" rel="noopener noreferrer">
                    <Button size="sm" variant="outline" className="border-border mt-3 gap-2"><Download className="h-3.5 w-3.5" />{t("smpCompanies:docs.downloadFile")}</Button>
                  </a>
                </div>
              )}
            </div>
...
function WorkersList({ companyId }: { companyId: string }) {
  ...
  function exportToExcel() {
    const x = (k: string) => t(`smpCompanies:workers.xlsx.${k}`);
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-white">{t("smpCompanies:workers.header")}</span>
        ...
        {workers.length > 0 && (
          <Button size="sm" variant="outline" className="border-border text-xs gap-1.5 h-7" onClick={exportToExcel} data-testid="button-export-workers">
            <Download className="h-3.5 w-3.5" />{t("smpCompanies:workers.export")}
          </Button>
        )}
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-8 border border-dashed border-border rounded-sm">
          ...
          <p className="text-xs text-muted-foreground">{t("smpCompanies:workers.empty")}</p>
        </div>
      ) : (
          ...
          {active.length > 0 && (
            <>
              <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-semibold">
                {t("smpCompanies:workers.active", { count: active.length, replace: { count: formatNumber(active.length, i18n.language) } })}
              </p>
          ...
          {inactive.length > 0 && (
            <>
              <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-semibold mt-3">
                {t("smpCompanies:workers.terminated", { count: inactive.length, replace: { count: formatNumber(inactive.length, i18n.language) } })}
              </p>
...
function WorkerCard({ worker }: { worker: WorkerRow }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <p className="text-sm font-medium text-white truncate"><bdi>{worker.fullNameEn ?? t("smpCompanies:workers.unknown")}</bdi></p>
          {worker.isActive
            ? <Badge className="text-[9px] bg-emerald-900/40 text-emerald-400 border-0 shrink-0">{t("smpCompanies:workers.activeBadge")}</Badge>
            : <Badge className="text-[9px] bg-zinc-800 text-zinc-400 border-0 shrink-0">{t("smpCompanies:workers.terminatedBadge")}</Badge>}
        </div>
...
function CompanySheet({
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["/api/smp-companies"] });
      setEditMode(false);
      toast({ title: t("smpCompanies:sheet.saveSuccess") });
    },
    onError: () => toast({ title: t("smpCompanies:create.errorTitle"), description: t("smpCompanies:sheet.saveError"), variant: "destructive" }),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["/api/smp-companies"] });
      onOpenChange(false);
      toast({ title: t("smpCompanies:sheet.deleteSuccess") });
    },
    onError: () => toast({ title: t("smpCompanies:create.errorTitle"), description: t("smpCompanies:sheet.deleteError"), variant: "destructive" }),
  });
  ...
  const toggleActiveMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["/api/smp-companies"] });
      toast({ title: company!.isActive ? t("smpCompanies:sheet.deactivated") : t("smpCompanies:sheet.activated") });
    },
    onError: () => toast({ title: t("smpCompanies:create.errorTitle"), description: t("smpCompanies:sheet.toggleError"), variant: "destructive" }),
  });
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <Button
                ...
                title={company.isActive ? t("smpCompanies:sheet.deactivate") : t("smpCompanies:sheet.activate")}
              ...
              <Button size="sm" variant="outline" className="border-border text-xs" onClick={editMode ? () => setEditMode(false) : startEdit} data-testid="button-edit-company">
                {editMode ? t("smpCompanies:sheet.cancel") : t("smpCompanies:sheet.edit")}
              </Button>
            </div>
          </div>
          <div className="flex border-b border-border mt-2 -mb-4">
            <button className={tabClass("info")} onClick={() => setActiveTab("info")} data-testid="tab-info">{t("smpCompanies:sheet.tabInfo")}</button>
            <button className={tabClass("docs")} onClick={() => setActiveTab("docs")} data-testid="tab-docs">{t("smpCompanies:sheet.tabDocs")}</button>
            <button className={tabClass("workers")} onClick={() => setActiveTab("workers")} data-testid="tab-workers">{t("smpCompanies:sheet.tabWorkers")}</button>
          </div>
        </SheetHeader>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2 space-y-1.5">
                    <Label className="text-white text-sm">{t("smpCompanies:create.name")} *</Label>
                ...
                <div className="flex gap-2 justify-between pt-2">
                  <Button variant="outline" className="border-red-900/60 text-red-400 hover:bg-red-950/40 text-xs gap-1.5" onClick={() => deleteMutation.mutate()} disabled={deleteMutation.isPending} data-testid="button-delete-company">
                    <Trash2 className="h-3.5 w-3.5" />{t("smpCompanies:sheet.delete")}
                  </Button>
                  <div className="flex gap-2">
                    <Button variant="outline" className="border-border text-xs" onClick={() => setEditMode(false)}>{t("smpCompanies:sheet.cancel")}</Button>
                    <Button className="bg-primary text-primary-foreground text-xs font-bold" onClick={() => updateMutation.mutate(editForm)} disabled={updateMutation.isPending || !editForm.name?.trim()} data-testid="button-save-company">
                      {updateMutation.isPending && <Loader2 className="h-3.5 w-3.5 me-1 animate-spin" />}{t("smpCompanies:sheet.save")}
                    </Button>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                {company.notes && (
                  <div className="space-y-1">
                    <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">{t("smpCompanies:sheet.notes")}</p>
                    <p className="text-sm text-muted-foreground leading-relaxed whitespace-pre-wrap"><bdi>{company.notes}</bdi></p>
                  </div>
                )}
                <div className="text-[11px] text-muted-foreground pt-2 border-t border-border">
                  {t("smpCompanies:sheet.createdOn", { date: formatGregDate(company.createdAt, i18n.language) })}
                </div>
              </div>
            )
          )}
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-display font-bold text-white tracking-tight">{t("smpCompanies:page.title")}</h1>
            <p className="text-muted-foreground mt-1">{t("smpCompanies:page.subtitle")}</p>
          </div>
          <Button className="h-11 bg-primary text-primary-foreground font-bold uppercase tracking-wide text-xs" data-testid="button-create-smp" onClick={() => setCreateOpen(true)}>
            <Plus className="me-2 h-4 w-4" />{t("smpCompanies:page.addBtn")}
          </Button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card className="bg-card border-border shadow-sm border-s-4 border-s-amber-500">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground uppercase tracking-wider">{t("smpCompanies:stats.total")}</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card className="bg-card border-border shadow-sm border-s-4 border-s-primary">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground uppercase tracking-wider">{t("smpCompanies:stats.active")}</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card className="bg-card border-border shadow-sm border-s-4 border-s-blue-500">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground uppercase tracking-wider">{t("smpCompanies:stats.smpWorkers")}</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card className="bg-card border-border shadow-sm border-s-4 border-s-zinc-500">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground uppercase tracking-wider">{t("smpCompanies:stats.regions")}</CardTitle>
            </CardHeader>
            <CardContent>
        ...
        <div className="flex items-center bg-card p-4 rounded-sm border border-border">
          <div className="relative flex-1 w-full">
            ...
            <Input
              placeholder={t("smpCompanies:search.placeholder")}
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center border border-dashed border-border rounded-sm">
            ...
            <p className="text-muted-foreground font-medium">
              {search ? t("smpCompanies:search.noMatch") : t("smpCompanies:search.noCompanies")}
            </p>
            {!search && (
              <p className="text-muted-foreground/60 text-sm mt-1">{t("smpCompanies:search.noCompaniesHelp")}</p>
            )}
          </div>
        ) : (
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            {company.crNumber && <p className="text-[11px] text-muted-foreground font-mono" dir="ltr">{t("smpCompanies:card.crPrefix")} {company.crNumber}</p>}
          </div>
        </div>
        <div className="flex flex-col items-end gap-1 shrink-0">
          <Badge className={`text-[10px] border-0 ${company.isActive ? "bg-emerald-900/40 text-emerald-400" : "bg-zinc-800 text-zinc-400"}`}>
            {company.isActive ? t("smpCompanies:card.active") : t("smpCompanies:card.inactive")}
          </Badge>
